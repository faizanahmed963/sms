...
if(isset($_GET['rs_id']))
	...
	if($opr=="del")
{
	$del_sql=mysqli_query($db,"DELETE FROM stu_tbl WHERE stu_id=$id");
	if($del_sql) {
        {
            echo "<div>"
...
<form method="post">
	...
	if($key !="")
		$sql_sel=mysqli_query($db,"SElECT * FROM stu_tbl WHERE f_name  like '%$key%' or l_name like '%$key%'");
	else
		 $sql_sel=mysqli_query($db,"SELECT * FROM stu_tbl");
